...
@implementation AlbumInfoCellUserData
@end
...
- (BOOL)shouldUpdateCellWithObject:(NICellObject *)object {
  self.userData = (AlbumInfoCellUserData *)object.userInfo;
  self.albumName.text = self.userData.album.name;
  self.albumDesc.text = self.userData.album.desc;
  NSString *url = QINIU_FILE_URL_MEDIUM(self.userData.album.cover);
...
+ (NICellObject *)createObject:(id)_delegate userData:(id)_userData {
  ...
  NICellObject *cellObj =
      [[NICellObject alloc] initWithCellClass:[AlbumInfoCell class]
                                     userInfo:userData];
  return cellObj;
}
@end

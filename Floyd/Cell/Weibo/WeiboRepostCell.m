...
@implementation WeiboRepostCellUserData
@end

@interface WeiboRepostCell ()
...
@implementation WeiboRepostCell
...
+ (CGFloat)heightForObject:(id)object
  ...
  WeiboRepostCellUserData *userData = (WeiboRepostCellUserData *)obj.userInfo;
  CGFloat maxWidth = tableView.frame.size.width - 30;

  CGFloat height = [userData.weiboRepost.text
...
- (BOOL)shouldUpdateCellWithObject:(NICellObject *)object {
  self.userData = (WeiboRepostCellUserData *)object.userInfo;
  self.userName.text = self.userData.weiboRepost.user.name;
  [self.avatarImage
      sd_setImageWithURL:[NSURL URLWithString:self.userData.weiboRepost.user
                                                  .profile_image_url]
        placeholderImage:[UIImage imageNamed:@"avatar_user_man"]];
  NSString *dateAndFromStr = [NSString
      stringWithFormat:@"%@ %@",
                       [self.userData.weiboRepost getCreatedAtString],
                       self.userData.weiboRepost.source];
  ...
  self.desc.text = self.userData.weiboRepost.text;
  return YES;
}

+ (NICellObject *)createObject:(id)_delegate userData:(id)_userData {
  WeiboRepostCellUserData *userData = [[WeiboRepostCellUserData alloc] init];
  NICellObject *cellObj =
      [[NICellObject alloc] initWithCellClass:[WeiboRepostCell class]
                                     userInfo:userData];
  return cellObj;
}
@end
